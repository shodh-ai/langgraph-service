...
try:
    if 'vertexai' in globals() and not hasattr(vertexai, '_initialized'):
        ...
        logger.info("Vertex AI initialized in default_fallback_node")
except Exception as e:
    logger.error(f"Failed to initialize Vertex AI in default_fallback_node: {e}")

try:
    gemini_model = GenerativeModel("gemini-1.5-pro")
    logger.info("Gemini model loaded in default_fallback_node")
except Exception as e:
    logger.error(f"Failed to load Gemini model in default_fallback_node: {e}")
    gemini_model = None
