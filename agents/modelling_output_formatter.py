...
async def modelling_output_formatter_node(state: AgentGraphState) -> dict:
    ...
    """
    logger.info("---Executing Simplified Modelling Output Formatter ---")

    payload = state.get("intermediate_modelling_payload")
    if not payload or "sequence" not in payload:
        return {"final_text_for_tts": "I lost my train of thought. Let's try again.", "final_ui_actions": []}
    ...
    for step in sequence:
        ...
        if action_type_str == "UPDATE_TEXT_CONTENT":
            ...
            # For all other actions, the step_payload itself becomes the parameters
            params = step_payload
