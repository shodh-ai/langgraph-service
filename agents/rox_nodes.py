...
async def rox_propose_plan_node(state: AgentGraphState) -> dict:
    """
    Takes data from the state (generated by the navigator) and formats it
    into the final user-facing output for this turn.
    """
    logger.info("--- Executing Rox Propose Plan Node (Presenter) ---")
    
    # Add this logging statement for definitive debugging
    logger.info(f"Proposer received state with keys: {list(state.keys())}")
    ...
    if not course_map_data:
        logger.error("CRITICAL: rox_propose_plan_node did NOT receive 'course_map_data' from the state.")
        return {
            "final_text_for_tts": "I'm having trouble retrieving the course map right now. Let's try that again.",
            "final_ui_actions": []
        }

    # Your existing logic for deciding the speech is perfect
    speech = ""
    if intent == "REQUEST_COURSE_MAP":
        speech = "Of course. Here is your learning map."
    else:
        speech = next_lo_info.get("proposal_script", "I've found the next step for us.")

    return {
        "final_text_for_tts": speech,
        "final_ui_actions": [
            {
                "action_type": "DISPLAY_COURSE_MAP",
                "parameters": { "map_data": course_map_data }
            },
            {
                "action_type": "SPEAK_THEN_LISTEN",
                "parameters": { "text_to_speak": speech }
            }
        ],
