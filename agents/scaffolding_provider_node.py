...
try:
    if 'vertexai' in globals() and not hasattr(vertexai, '_initialized'):
        ...
        logger.info("Vertex AI initialized in scaffolding_provider_node")
except Exception as e:
    logger.error(f"Failed to initialize Vertex AI in scaffolding_provider_node: {e}")

try:
    gemini_model = GenerativeModel("gemini-1.5-pro")
    logger.info("Gemini model loaded in scaffolding_provider_node")
except Exception as e:
    logger.error(f"Failed to load Gemini model in scaffolding_provider_node: {e}")
    gemini_model = None
