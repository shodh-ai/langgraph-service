...
class InteractionRequestContext(BaseModel):
    ...
    ui_event_payload: Optional[Dict[str, Any]] = None # e.g., data from a form field on P9 drill
...
class ReactUIAction(BaseModel): # RENAMED from DomAction
    ...
    target_element_id: Optional[str] = None # ID of the HTML element for actions like UPDATE_TEXT_CONTENT
    parameters: Optional[Dict[str, Any]] = Field(default_factory=dict) # RESTRUCTURED from 'payload'.
...
class InteractionResponse(BaseModel):
    ...
    ui_actions: Optional[List[ReactUIAction]] = Field(default_factory=list) # RENAMED from 'dom_actions'. Uses the new ReactUIAction model.
    
    # Optional fields for guiding frontend flow or providing additional info
    next_task_info: Optional[Dict[str, Any]] = None # e.g., {"type": "SPEAKING", "prompt_id": "SPK_Q1_CITY", "title": "Your Favorite City"}
    navigation_instruction: Optional[Dict[str, Any]] = None # More structured than just a string
